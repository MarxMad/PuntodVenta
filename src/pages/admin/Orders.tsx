  ...
  return (
    <div onClick={onClose} style={overlay}>
      <div onClick={(e) => e.stopPropagation()} className="cap-pop" style={modal}>
        <div style={{ fontFamily: font.display, fontWeight: 700, fontSize: 20, color: C.text, marginBottom: 16 }}>Nuevo pedido</div>
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar producto…" style={{ ...inputStyle, marginBottom: 10 }} />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 260, overflowY: 'auto' }}>
          {filtered.map((p) => (
            <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, background: C.bg, borderRadius: 11, padding: '9px 12px' }}>
              ...
              <button onClick={() => setQty(p.id, (items[p.id] ?? 0) - 1)} style={miniBtn}>−</button>
